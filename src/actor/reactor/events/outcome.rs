...
pub(crate) struct EventOutcome {
    ...
    /// Whether the layout-changed broadcast should go out. Set by
    /// `layout_changed`, which is the constructor that says one did.
...
impl EventOutcome {
    ...
    pub(crate) fn absorb(&mut self, mut other: Self) {
        ...
        self.post_arrange_focus_warp =
            other.post_arrange_focus_warp.or(self.post_arrange_focus_warp);
    ...
    pub(crate) fn layout_changed(is_resize: bool) -> Self {
        Self {
            // Not covered by `..Self::default()`: the derive gives `false`, and
            // this constructor is the one that says a layout change happened.
            // Upstream's tidy-up dropped the explicit field list, which would
            // have turned every layout broadcast off without a word.
