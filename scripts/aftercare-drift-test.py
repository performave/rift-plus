...
`record_aftercare` acts per cycle and checks, after the last cycle, that no
window is left in no tree.
    ...
    for space in all_space_ids(rift("displays")):
        for w in rift("windows", "--space-id", str(space)) or []:
            if w.get("app_name") not in ("Safari", "TextEdit"):
                continue
            # On a desktop nobody is showing is fine: the scenario this runs
            # after leaves the external showing an empty desktop and the
            # windows on its other one, and an unplug keeps them there. What
            # is not fine is a window in no tree.
            if not is_tiled(w) and not w.get("is_floating"):
                bad.append((w["id"]["idx"], w.get("app_name"), space, "in no tree"))
    print(f"  shown desktops {sorted(s for s in on if s)}; aftercare sends over {cycles} cycles: {total}")
    if bad:
        print(f"FAIL  windows left in no tree: {bad}")
        return 1
    if total > 1:
        print(f"FAIL  aftercare kept resending ({total} sends over {cycles} cycles)")
        return 1
    print("PASS  every window in a tree; aftercare did not keep resending")
    return 0
